...
export function createHealthBar(
  ...
  companionsSegments: number = 0,
  strangersSegments: number = 0,
): Item[] {
  ...
  const healthBackgroundColor = "black";
  ...
  const healthFillPortion = group === "COMPANIONS" ? getFillPortion(health, maxHealth, companionsSegments) : getFillPortion(health, maxHealth, strangersSegments);

  // const healthColor = 3*health <= maxHealth ? "#880b0a" : (3*health <= 2*maxHealth ? "#ac0100" : "#d02200");
  const healthColor = "#d02200";
...
const NAME_BACKGROUND_OPACITY = 0.7;
...
export function createNameTag(
  ...
  const nameTagText = buildLabel()
    ...
    .padding(sceneDpi / 50)
    .backgroundOpacity(NAME_BACKGROUND_OPACITY)
    .locked(LOCKED)
