...
async function refreshAllHealthBars() {
  ...
  const mode = await getModeFromScene();
  for (const item of items) {
    createAttachments(mode, item, playerId, playerRole, sceneDpi);
  }
...
async function startCallbacks() {
  if (!callbacksStarted) {
    ...
    lastMode = await getModeFromScene();
    const unSubscribeFromSceneMetadataMode = OBR.scene.onMetadataChange(async (sceneMetadata) => {
      const mode = await getModeFromScene(sceneMetadata);
      if (lastMode !== mode) {
        refreshAllHealthBars();
        lastMode = mode;
      }
    ...
    const unsubscribeFromScene = OBR.scene.onReadyChange((isReady) => {
      if (!isReady) {
        ...
        unSubscribeFromSceneMetadataMode();
...
function createAttachments(mode: "BATTLE" | "EXPLORE", item: Image, id: string, role: "PLAYER" | "GM", dpi: number) {
  ...
  if (mode === "EXPLORE" || (role === "PLAYER" && id != item.createdUserId && healthVisibility === "HIDDEN")) {
