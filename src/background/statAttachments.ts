...
let settings: Settings = {
  verticalOffset: 0,
  barAtTop: false,
  companionsSegments: 0,
  strangersSegments: 0,
  nameTags: false,
};
let callbacksStarted = false;
let userRoleLast: "GM" | "PLAYER";
let lastStatsVisibility: "BATTLE" | "EXPLORE";
...
async function refreshAllHealthBars() {
  ...
  const statsVisibility = await getStatsVisibilityFromScene();
  for (const item of items) {
    createAttachments(statsVisibility, item, playerId, playerRole, sceneDpi);
  }
...
async function startCallbacks() {
  if (!callbacksStarted) {
    ...
    lastStatsVisibility = await getStatsVisibilityFromScene();
    ...
    const unsubscribeFromSceneMetadata = OBR.scene.onMetadataChange(
      async (metadata) => {
        ...
        const statsVisibility = await getStatsVisibilityFromScene(metadata);
        if (lastStatsVisibility !== statsVisibility) {
          refreshAllHealthBars();
          lastStatsVisibility = statsVisibility;
        }
...
function getChangedItems(imagesFromCallback: Image[]) {
  ...
  for (let i = 0; i < imagesFromCallback.length; i++) {
    if (i > itemsLast.length - s - 1) {
      ...
      //check position, visibility, owner and metadata changes
      !(
        ...
        itemsLast[i + s].createdUserId === imagesFromCallback[i].createdUserId &&
...
function createAttachments(statsVisibility: "BATTLE" | "EXPLORE", item: Image, id: string, role: "PLAYER" | "GM", dpi: number) {
  const { origin, bounds } = getOriginAndBounds(settings, item, dpi);
  
  // Create stats
  const [health, maxHealth, tempHealth, armorClass, healthVisibility, group] =
    getTokenStats(item);
  
  if (statsVisibility === "EXPLORE" || (role === "PLAYER" && id != item.createdUserId && healthVisibility === "HIDDEN")) {
  ...
  } else if (role === "PLAYER" && id != item.createdUserId && healthVisibility === "BAR") {
  ...
  if (settings.nameTags && plainText !== "") {
    ...
    if (settings.barAtTop) {
      if (
        maxHealth <= 0 ||
        (role === "PLAYER" && id != item.createdUserId && healthVisibility === "HIDDEN")
      ) {
        nameTagPosition.y = origin.y - 4;
      } else if (role === "PLAYER" && id != item.createdUserId && healthVisibility === "HIDDEN") {
  ...
  function createFullHealthBar() {
    ...
    addItemsArray.push(
      ...createHealthBar(item, bounds, health, maxHealth, group, origin),
    );
    return true;
  }
