...
import { Mode } from "@/action/types";
import { getModeFromScene } from "@/action/helpers";
...
function createAttachments(mode: Mode, item: Image, id: string, role: "PLAYER" | "GM", dpi: number) {
  const { origin, bounds } = getOriginAndBounds(settings, item, dpi);
  
  // Create stats
  const [health, maxHealth, tempHealth, armorClass, healthVisibility, team] =
    getTokenStats(item);
  ...
  function createFullHealthBar() {
    ...
    addItemsArray.push(
      ...createHealthBar(item, bounds, health, maxHealth, team, origin),
    );
    return true;
  }
