...
export default function BulkEditor(): JSX.Element {
  ...
  useEffect(() => {
      if (playerRole === "PLAYER") {
        OBR.action.setHeight(439)
        OBR.action.setWidth(318)
      } else {
        OBR.action.setHeight(572)
        OBR.action.setWidth(392)
      }
    }, [playerRole]
  )
  ...
  const updateTokens = (items: Item[]) => {
    console.log("UT items", items);
    const newTokens = parseItems(items);
    console.log("UT parse items", newTokens);
    ...
    for (let i = 0; i < newTokens.length; i++) newTokens[i].index = i;
    setTokens(newTokens);
    console.log("UT newTokens", newTokens);
  };
  ...
  useEffect(() => {
    OBR.scene.onMetadataChange(async (sceneMetadata) => {
      getModeFromScene(sceneMetadata).then((mode) =>
        dispatch({
          type: "set-mode",
          mode: mode,
        })
