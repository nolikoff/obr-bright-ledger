  ...
  OBR.scene.items.updateItems(
    ...
    (items) => {
      for (let i = 0; i < items.length; i++) {
        ...
        console.log("RM ", retrievedMetadata);

        const combinedMetadata = { ...retrievedMetadata, ...newMetadata }; //overwrite only the modified value
        
        console.log("CM ", combinedMetadata);
...
const MODE_METADATA_ID = getPluginId("mode");
...
export async function getRollsFromScene(sceneMetadata?: Metadata) : Promise<StampedDiceRoll[]> {
  ...
  if (diceRolls === undefined)
    return [];
  // if (!isDiceRollArray(diceRolls))
  //   console.log("Error: invalid dice roll array!");

  // return diceRolls;
  return isDiceRollArray(diceRolls) ? diceRolls : [];
}

export async function setSceneMode(mode: Mode) {
  await OBR.scene.setMetadata({ [MODE_METADATA_ID]: mode });
}

export async function getModeFromScene(sceneMetadata?: Metadata) : Promise<Mode> {
  if (sceneMetadata === undefined)
    sceneMetadata = await OBR.scene.getMetadata();

  const mode = sceneMetadata[MODE_METADATA_ID];

  return isModeGood(mode) ? mode : "EXPLORE";
}

function isModeGood(mode: unknown): mode is Mode {
