...
import
  ChangeTeam
from "@/components/icons/ChangeTeam";
...
export function TokensTable({
  ...
  //   playerRole === "GM" 
  //   ? (appState.team === "COMPANIONS" 
  //     ? tokens.filter(token => token.team === "COMPANIONS")
  //     : tokens.filter(token => token.team === "STRANGERS")
  //   ) : tokens.filter(token => token.item.visible);
  ...
  return (
    <>
      ...
      {appState.operation === "DAMAGE" && playerRole === "GM" && (
        <GmDamageTable
          appState={appState}
          dispatch={dispatch}
          tokens={filteredTokens}
      ...
      {appState.operation === "HEALING" && playerRole === "GM" && (
        <GmHealingTable
          appState={appState}
          dispatch={dispatch}
          tokens={filteredTokens}
      ...
      {appState.operation === "OVERWRITE" && playerRole === "GM" && (
        <GmOverwriteTable
          appState={appState}
          dispatch={dispatch}
          tokens={filteredTokens}
                  ...
                  {tokens.length === 0 && (
                    ...
                    >       
                      <p className="text-muted-foreground text-sm">
                        This team has no tokens right now.<br/>
                        {
                          appState.team === "COMPANIONS" 
                          ? "When you add or move a token to the Companions, it will show up here."
                          : "When you add or move a token to the Strangers, it will show up here."
                        }
                      </p>
                    </div>
                  )}
                  {tokens.filter(token => appState.team === token.team ).map((token) => {
                          ...
                          >        
                            ...
                            >
                              <div className="flex max-w-32 items-center justify-center">
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      className="flex h-8 w-[40px] items-center justify-center text-lg font-medium"
                                      style={{
                                        padding: "0px",
                                      }}
                                      tabIndex={-1}
                            ...
                            >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div
                                      className="flex h-[32px] min-w-12 items-center justify-center rounded-md text-sm"
                                      style={{
                                        width: "80px",
                                      }}
                                    >
      ...
      <div className="flex w-full items-center gap-2 px-4 pt-2">
        <Tooltip>
          <TooltipTrigger asChild>
            <Input
              value={damageValue}
              className="bg-mirage-50 dark:bg-mirage-900"
              style={{
                height: "32px",
                textAlign: "center",
                width: `${Math.max(String(damageValue).length, 4)*14}px`,
              }}
              onChange = {(e) => setDamageValue(toValidInt(e.target.value))
              }
            />
          </TooltipTrigger>
          <TooltipContent>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Input
              ...
              onChange = {(e) => setHealingValue(toValidInt(e.target.value))
              }
            />
          </TooltipTrigger>
          <TooltipContent>
...
function GmOverwriteTable({
  ...
  const [valueHeal, setValueHeal] = useState<number>(appState.value ? appState.value : 0);

  useEffect(() => {
    setValueHeal(appState.value
      ? appState.value
      ?? valueHeal
      : 0
    );
  ...
  return (
    <>
      ...
      {/* <div className="flex gap-2 px-4 pt-2">
        <StatInput
          parentValue={valueHeal}
          name={"HP-HEAL"}
          updateHandler={(target) =>
            setValueHeal(toValidInt(target.value))
          }
        />
        <ActionButton
          label={"Overwrite"}
...
function PlayerTable({
  ...
  const playerTokensFilter = (token: Token) => token.item.createdUserId === OBR.player.id;
      ...
      <div className="flex flex-col items-center justify-start gap-2">
        <div>
          ...
          >
            <Table tabIndex={-1}>
              <TableBody>
                {tokens.filter(playerTokensFilter).length === 0 && (
                  <div
                    style={{
                      padding: "10px 16px 10px 16px",
                    }} 
                  >       
                    <p className="text-muted-foreground text-sm">
                      ...
                      When you become the owner of a token, it will show up here.
                    </p>
                  </div>
                )}
                {tokens.filter(playerTokensFilter).map((token) => {
                  const included = getIncluded(
                    token.item.id,
                    appState.includedItems,
                  );
...
async function updateTokenHealthVisibility(
  ...
  const name: InputName = "HEALTH-VISIBILITY";
  ...
  await OBR.scene.items.updateItems(
    ...
    (items) => {
      for (let i = 0; i < items.length; i++) {
        ...
        const newMetadata = {
          [HEALTH_VISIBILITY_METADATA_ID]: included ? newHealthVisibility : tokens[i].healthVisibility,
        };
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size={"icon"}
            onClick={() =>
              updateTokenTeam(token.item.id, token.team, setTokens)
            }
          >
            <ChangeTeam/>
          </Button>
        ...
        <TooltipContent side="top">
          {token.team === "COMPANIONS" ? "Move to Strangers" : "Move to Companions"}
        </TooltipContent>
      </Tooltip>
    </TableCell>
  );
...
async function updateTokenTeam(
  itemId: string,
  previousValue: "COMPANIONS" | "STRANGERS",
  setTokens: React.Dispatch<React.SetStateAction<Token[]>>,
) {
  const name: InputName = "TEAM";
  if (!isInputName(name)) throw "Error: invalid input name.";
...
export async function updateCheckboxedTokensTeam(
  includedItems: Map<string, boolean>,
  tokens: Token[],
) {
  await OBR.scene.items.updateItems(
    ...
    (items) => {
      for (let i = 0; i < items.length; i++) {
        ...
        const newTeam = tokens[i].team === "COMPANIONS" ? "STRANGERS" : "COMPANIONS";

        const newMetadata = {
          [TEAM_METADATA_ID]: included ? newTeam : tokens[i].team,
        };
